...
test('Validating Patient Card Fields', async ({ browser }) => {
...
test('Validating Procedure Date Sorting', async ({ browser }) => {
    const context = await browser.newContext();
    const page = await context.newPage();
    const dropDownArrow = page.locator(".MuiSvgIcon-root.MuiSvgIcon-fontSizeMedium.css-14h0jh8");
    await page.goto("https://hopper-frontend-cxk56rs2hq-uc.a.run.app//");
    ...
    const procedureDateFormatted = moment.utc(procedureDate).format("MMMM D, YYYY");
